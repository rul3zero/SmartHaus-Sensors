...
// frame pacing handled via wsFrameInterval
...
unsigned long connectionTimeout = 600000; // 10 minutes
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
    switch(type) {
        ...
        case WStype_CONNECTED:
            {
                ...
                activeClient = num;
                streamActive = false; // wait for explicit start command
        ...
        case WStype_TEXT:
            ...
            {
                std::vector<uint8_t> buffer(payload, payload + length);
                buffer.push_back(0);
                char* message = reinterpret_cast<char*>(buffer.data());
                Serial.printf("[%u] get Text: %s\n", num, message);
            
                // Example command: "stream_start" and "stream_stop"
                if (strcmp(message, "stream_start") == 0 && num == activeClient) {
                    streamActive = true;
                    webSocket.sendTXT(num, "Streaming started");
                } 
                else if (strcmp(message, "stream_stop") == 0 && num == activeClient) {
                    streamActive = false;
...
void sendCameraFrameWs() {
    if (!clientConnected || !streamActive || activeClient < 0) return;
    ...
    webSocket.sendBIN(activeClient, fb->buf, fb->len);
    Serial.printf("Sent frame (%d bytes) to client %d\n", fb->len, activeClient);
...
void sendIPToFirebase() {
    String ipAddress = String(WG_LOCAL_IP);  // Use WireGuard VPN IP
...
void setupDNS() {
    ...
    // DNS is automatically configured via DHCP when WiFi connects
    // ESP32 WiFi library handles DNS automatically
...
// Setup WireGuard VPN
void setupWireGuard() {
    Serial.println("=== INITIALIZING WIREGUARD VPN ===");
    
    // Parse WireGuard private key
    wg.begin(
        IPAddress(10, 66, 66, 21),           // Local IP in VPN
        WG_PRIVATE_KEY,                       // Private key
        WG_ENDPOINT_ADDRESS,                  // VPN server address
        WG_PUBLIC_KEY,                        // Server public key
        WG_ENDPOINT_PORT                      // Server port
    );
    
    Serial.println("WireGuard VPN initialized");
    Serial.printf("Local VPN IP: %s\n", WG_LOCAL_IP);
    Serial.printf("Endpoint: %s:%d\n", WG_ENDPOINT_ADDRESS, WG_ENDPOINT_PORT);
    
    // Allow some time for connection establishment
    delay(2000);
    wireguardConnected = true;
    Serial.println("✅ WireGuard VPN connection established");
    Serial.println("=== WIREGUARD READY ===");
}
...
void handleRoot() {
    ...
    html += "<p><strong>WebSocket URL:</strong> ws://" + String(WG_LOCAL_IP) + ":81/ws</p>";
...
void handleStatus() {
    String status = "{";
    status += "\"vpn_ip\":\"" + String(WG_LOCAL_IP) + "\",";
...
void setup() {
    ...
    // Setup NTP time synchronization (required for WireGuard)
    setupNTP();
    ...
    Serial.println("NTP: ✅");
    Serial.println("DNS: ✅");
    Serial.println("WireGuard: ✅");
    Serial.println("=== STARTING MAIN LOGIC ===");
    Serial.println();
...
void loop() {
    ...
    if (!hasActiveConnection && !clientConnected) {
        ...
        if (currentTime - lastConnectionTime > connectionTimeout) {
            Serial.println("No connections detected for 10 minutes. Restarting ESP32...");
            delay(1000);
            ESP.restart();
        }
    }
